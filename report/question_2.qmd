...
ggplot(distr_nightstays_per_district, aes(x = F20, y = F105_OrgCode, group = F105_OrgCode)) + 
  geom_density_ridges(fill = "#00AFBB", color = "white", alpha = 0.8) +  # Adjust alpha for transparency
  scale_x_continuous(labels = scales::comma) +  # Add comma separator to x-axis labels for better readability
  labs(
...
# Replace NA values with 0 in column F20
distr_nightstays_per_district$F20[is.na(distr_nightstays_per_district$F20)] <- 0

distr_nightstays_per_district <- distr_nightstays_per_district %>%
  group_by(F105_OrgCode) %>% 
  summarise(count = n(), .groups = 'drop')
  
  
distr_nightstays_per_district %>% ifelse(is.na(distr_nightstays_per_district$F20), 0, distr_nightstays_per_district$F20) # Replace NA values with 0 in column F20 using ifels
