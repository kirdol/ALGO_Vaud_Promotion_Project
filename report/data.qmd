...
We can clearly see that some columns have a high percentage of missing values. Some of these missing values are due to questions that are answered only if the person completing the survey does not find the correct answer among the proposed multiple-choice options. Therefore, they need to input their own custom answer. F82_0x columns are about the expsenses of the tourists if they took a package. F81_0x columns are linked to the expenses of the tourists that did not took a package. These column are thus complementary (`r round(sum(is.na(data$F82_01)) / nrow(data) * 100)`% + `r round(sum(is.na(data$F81_01)) / nrow(data) * 100)`% = `r (sum(is.na(data$F82_01)) / nrow(data) * 100) + (sum(is.na(data$F81_01)) / nrow(data) * 100)`%). This needs to be kept in mind when interpreting the results of our analysis