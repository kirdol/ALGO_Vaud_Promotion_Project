...
Below is the list of questions that were asked during the survey. The original questionnaire that we received was in German. The questions were translated before we conducted our analysis. The version shown below is a simplified version of the original questions. Find the original German and English explanations in [Annex -@sec-data-description] and a better view of the type of data in [Annex -@sec-data-summary].
...
Now that we have a better understanding of the questions asked during the survey, let's look at the missing values in our dataset. Find below the visualization of the entire dataset. The gray cells represent the missing values.
...
# Generate the plot with the custom color palette
plot <- vis_dat(data) +
  scale_fill_manual(values = discrete_color_less) +
...
```{r Vis_miss Missing values, echo=FALSE, message = FALSE}
...
discrete_color_NA <- c("#d81f1c", "#7f7f7f")

# Generate the plot with the custom color palette
plot <- vis_miss(data_ordered) +
  scale_fill_manual(
    name = "Data Status",
    values = discrete_color_NA,
...
It's evident that several columns exhibit a high rate of missing data. This often occurs when respondents are required to enter a custom answer because the provided multiple-choice options do not include an appropriate response. The F82_0x columns relate to the expenses incurred by tourists who opted for a package, while the F81_0x columns pertain to expenses of tourists who did not purchase a package. These columns are thus complementary (`r round(sum(is.na(data$F82_01)) / nrow(data) * 100)`% + `r round(sum(is.na(data$F81_01)) / nrow(data) * 100)`% = `r (sum(is.na(data$F82_01)) / nrow(data) * 100) + (sum(is.na(data$F81_01)) / nrow(data) * 100)`%). This needs to be kept in mind when interpreting the results of our analysis.