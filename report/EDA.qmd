...
Now in order to know more precisely about the destination of the visitors, we decided to separate countries of origin for each city. For visualization purpose, we decided to take the top 10 most visited cities in the dataset. You will find the full table with the occurences per nationality in the [Annex -@sec-data-city_percentage_continent].
...
custom_colors <- brewer.pal(num_colors, "Spectral")  # Use 'Paired' palette
...
As mentioned above, more than 30% of the visitors surveyed were Swiss. We therefore decided to find out more about the percentage of domestic visitors in the canton of Vaud. Here is the distribution of visitors by canton of origin, scaled by taking into account the population size of the cantons. This ensure the comparison between the different cantons. The full table can be found at [Annex -@sec-data-domestic_visitors]
