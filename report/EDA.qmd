...
As can be seen from this table, the cantons with the highest percentage of domestic visitors scaled by the total population of these cantons are Vaud (approx. 8.5%), Uri (approx. 7.3%) and Obwalden (approx. 5.8%).

```{r, fig.width=10, fig.height=6, echo=FALSE, warning=FALSE}
#### Distribution of domestic tourism - German speaking part

# As we know that around 65% of Switzerland has German as its main language, we thought it would be interesting to know the distribution of Swiss-German visitors in the survey. Here is the graph showing the distribution per canton, again scaled by its population.


# Now how much percentage do swiss germans represent in this
# Swiss_germans_percentage <- data_merged %>%
...
# 
# # plot scaled_ratio per canton
# ggplot(Swiss_germans_percentage,
#        aes(x=reorder(Canton, Scaled_Ratio),
#            y=Scaled_Ratio)) +
#   geom_bar(stat="identity",
#            width = 0.8) +
#   labs(title="Distribution among swiss german visitors (scaled ratio)",
#        x="Cantons",
#        y="Percentage") +
#   theme(legend.title = element_blank(),
#         legend.position = "none",
