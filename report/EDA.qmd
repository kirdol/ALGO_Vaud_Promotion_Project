...
The survey gathered around 2'500 responses from 100 different nationalities. We decided to start by looking at the overall nationality distribution of the visitors. The full statistics and additional information can be found in the [Annex -@sec-data-distributionnationality] section.
...
```{r, warning=FALSE, out.height="100%", out.width="100%", fig.width=7, echo=FALSE}
### F05_01.ENG ###
...
ggplot(F05_01_PCT,
       aes(x=reorder(Continent_of_origin, Percentage*100),
           y=Percentage*100)) +
  geom_bar(stat="identity", fill=default_color) +  # Set your desired color here
  coord_flip() +
  theme_minimal() +
  labs(x="Continents",
       y="Percentages",
  ...
  geom_text(aes(label=sprintf("%.2f%%", Percentage*100)),
            ...
            size = 3) +
  ylim(0, 90)
  #scale_fill_brewer(palette = "discrete_color")
```
...
ggplot(F05_02_PCT, aes(x = reorder(`F05_02_ENG`, Percentage), y = Percentage, fill = `F05_01_ENG`)) + 
  ...
  coord_flip() +
  scale_fill_manual(values = brewer.pal(5, "RdYlGn"), name = "Continent") +
  theme_minimal() +
...
Our results show that Switzerland is the country most represented among all visitors (around 31.5%). The canton of Vaud has a large number of domestic visitors. We can also see that the countries most represented are European, such as France (around 14.5%), the United Kingdom (around 7.5%), Germany (around 5.5%), Belgium (around 3.5%), and finally Spain (around 3%). The only other nationality from another continent represented here is the United States of America (around 5.5%). Proximity to Switzerland could, for instance, be one of the main factors explaining these results.

## City and district visitor distributions

### Percentage of each country of origin by city

To gain a more precise understanding of the visitors’ destinations, we have separated the countries of origin for each city. For visualization purposes, we decided to take the top 10 most visited cities in the dataset. You will find the full table with the occurrences per nationality in the [Annex -@sec-data-city_percentage_continent].
...
num_colors <- length(unique(final_data$F05_02_ENG))
custom_colors <- brewer.pal(num_colors, "RdYlGn")  # Use 'Paired' palette
...
Here, the percentage is, in fact, a ratio representing the number of people from a particular nationality over the total number of visitors in each city. Unfortunately, very few data has been collected in some cities, the ratio of visitor from one nationality over all the visitor surveyed can be really large (up to 1, in point of percentage). We notice in blue duck that Switzerland is again the main tourist class in most of the represented cities here, followed by visitors from the United Kingdom and France. On the other hand, as data from some other city gathered a lot of answers, we observe that some country of origin (e.g. Austria in Lausanne) only account for 0.2% of the total number of visitors.

### Ratio of each country relativel to the number of tourist surveyed in each district

In the same process, let's now explore the country of origin ratio per district of tourism. The full table is in the [Annex -@sec-data-city_percentage_country].
...
plot <- plot_ly(nationality_ratio_per_city, 
                x = ~District_tourism, 
                y = ~Ratio * 100, 
                color = ~Country_of_origin,
                colors = custom_colors,
                type = 'bar') %>%
  layout(title = 'Stacked Bar Plot of Ratios by District and Nationality',
         xaxis = list(title = 'City', tickangle = -45),
         yaxis = list(title = 'Percentages'),
         barmode = 'stack',
         showlegend = FALSE)  # Remove the legend
...
Again, the low number of data available in our data doesn't allow us to get really satisfying results for every district surveyed. Nevertheless, we notice the same results as the previous graph. 

### Domestic tourism

#### Global Swiss domestic visitors

As mentioned above, more than 30% of the visitors surveyed were Swiss. We therefore decided to investigate more about the percentage of domestic visitors in the canton of Vaud. Here is the distribution of visitors by canton of origin, scaled by canton population size. This ensures a scaled comparison between different cantons. The full table can be found in [Annex -@sec-data-domestic_visitors] section.
...
ggplot(Inside_Switzerland_ratio, aes(x = reorder(Canton, Scaled_Ratio), y = Scaled_Ratio)) +
  geom_bar(stat = "identity", width = 0.8, fill = default_color) +
  labs(title = "Scaled Distribution of Domestic Visitor Cantons", x = "Cantons", y = "Percentages") +
  theme_minimal() +
  theme(
...
As it can be seen from this plot, the cantons with the highest percentage of domestic visitors scaled by the total population of these cantons are Vaud (around 8.5%), Uri (around 7.3%) and Obwalden (around 5.8%).

For the sake of readability and better visualization, we decided to group the cantons into three categories: Swiss-Germans, Vaud, and the remaining cantons under the category "Other". Here is the scaled distribution of the swiss population surveyed:
...
ggplot(counts,
  ...
  geom_bar(stat = "identity", width = 0.9, fill = default_color) +
  theme_minimal() +
...
When looking at the total number of domestic visitors, around 70% come from the German-speaking part (18 cantons), which is more than three time as many as the 6 cantons under the "Other" category (Fribourg, Geneva, Grisons, Jura, Ticino, Valais) and more than 8 times as many as visitors as the canton of Vaud. 
...
Where did they stay? (For example: yes, they went to Montreux, but what are the detail and the type of accommodation where they stayed?)
...
Let's now dive into the subject of the location of their stay. First, let's have a look at the most popular locations within the canton of Vaud.
...
plot_ly(resort_counts_df,
        ...
        hovertext = ~HoverText,
        textposition = 'auto',
        marker = list(color = default_color)) %>%
...
From this plot, we can see that the most popular city in the canton of Vaud is Lausanne. Let's now have a look at the type of accommodation where the visitor stayed in the canton of Vaud. We took the liberty to remove the `r data %>% filter(F125_ENG == "#N/A") %>% summarise(Count = n())` observations where the type of accommodation was not specified. We can now focus on the districts where people stayed.
...
As we can see, out of the `r n_distinct(data$F105_Code)` cities surveyed in the `r n_distinct(data$leaflet_districts)` districts, the most popular district is `r resort_counts_df$Var1[order(-resort_counts_df$Proportion)][1]`. This is not really surprising because of the way the data were collected. More people were surveyed in these city/districts than other smaller ones. It is still interesting to have a look while keeping this in mind.
...
The most popular accommodation type in the canton of Vaud, according to the survey of 2'440 respondents, is the Hotel/Spa House, selected by 777 individuals. Notably, 293 people chose to stay at campsites. For Swiss respondents specifically, campsites rise to the second most popular option, with nearly 30% opting for this type of accommodation, while the Hotel/Spa House remains the top choice. Interestingly, the free apartment/house is highly favored by visitors from France, the United Kingdom, Germany, and Belgium, ranking in their top two choices. Conversely, visitors from the United States tend to prefer hotels, rented apartments/houses, or other types of accommodations over free apartments. Additionally, campsites are notably less popular among American visitors. Below, we created a wordcloud that illustrates other favored types of accommodation.
...
As we can see on the graph above, the best average is obtained in `r combined_data$leaflet_districts[1]` with an average of `r combined_data$Average_F130_num[1]` stars. But these statistics are not very representative of reality. Only `r combined_data$Count[1]` people answered the survey in this district, which is not enough to prove the higher quality of hotels. The second-best region after `r combined_data$leaflet_districts[1]` is `r combined_data$leaflet_districts[2]` with an average of `r combined_data$Average_F130_num[2]` stars. This district received `r combined_data$Count[2]` observations, which is more representative. This high score for the region of `r combined_data$leaflet_districts[2]` is not really surprising given the fact that this region is well known for its luxury hotels.

## Spendings

This section concentrates on the expenses and the provenance of the tourists. It also dive into the time period they came and where did they go.

### Percentage of people with & without a package

To start, we can identify two types of traveler spending: those who visit Switzerland without a package, meaning they incur expenses across various categories not covered by any package, and those who travel with a package, which typically includes certain categories such as accommodation and food.
...
ggplot(spending_counts, aes(x = PackageType, y = Percentage)) +
  geom_bar(stat = "identity", fill = default_color) +
...
The majority of people traveling to the canton of Vaud come without a package. Despite only having around 8% of travelers with a package, it is of interest to us to see where they come from.
...
p <- ggplot(continent_summary, aes(x = reorder(F05_01_ENG, -Package_Tour_Percentage), y = Package_Tour_Percentage, fill = default_color)) +
  geom_bar(stat = "identity") +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "none") +
  scale_fill_manual(values = default_color)

# Display the plot
print(p)
```

As a result, we observe that the main spenders on packages are from Asia, Africa, and South America. It would be interesting to target travelers from these continents for package deals rather than the rest of the continents.
...
We observe that most of the expenses of the people coming to Vaud are their overnight stay, followed by their food expenses, transports, and souvenirs.
...
For the tourists that took a package, most is spent on the package, then the food expenses, and finally the overnight stay. This is explained by the fact that packages usually include transportation and activities, which have a pretty high cost in Switzerland.
...
Here, we will focus on the seven most represented countries in the data, putting the rest of the countries into a "other countries" category. By doing so, we can observe which countries have the most expenses among the main countries.
...
# Create the bar plot for F81_Total with continent colors
ggplot(total_spending_F81, aes(x = reorder(F05_02_ENG_grouped, -Total_Spending_F81), y = Total_Spending_F81, fill = F05_01_ENG)) +
  ...
  scale_fill_manual(values = discrete_color, name = "Continent") + # Color based on continent
  theme_minimal() +
...
Looking at the packages, and as we observed in the percentage of packages by continent above, Europeans and North Americans aren't the ones spending the most in total on packages. It is rather Asians who do so.
...
Looking at averages, North Americans spend the most on average when coming to the canton of Vaud, followed by Belgium and the United Kingdom.
...
Once again, looking at the average with a package, it is North Americans.

:::

For the plots above, we removed outliers (spending of more than 30'000 CHF) because these observations biased the rest of the sample. We assumed that it was probable that most respondents with spending above 30'000 made an error in the currency, using their local currency to answer the questions instead of CHF.
...
We observed that the most represented currency are Swiss Francs and Euros. Other currencies are less used. As mentioned above, we assumed that some people used their currencies when inputting their spending amounts, which created outliers in our results.
...
We notice that more than 2/3 of overall visitors came to the canton of Vaud with their partner (68.7%). Of these 68.7%, 43% came only with their partner, without friends, children or other family. Almost 1/3 of visitors came with their children. Only a small fraction of people visited the canton of Vaud alone (14.6%), and a minority of people took their pet(s) with them (6.5%).

# Methodology

After receiving the raw data, we cleaned and translated to english the questions and answers. We then exported is to a CSV data file for easier data manipulation in the different software used for this project. We then wrote the EDA (Exploratory Data Analysis) section to display single variables and responses as graphs and plots to get a better idea of the scale and type of responses in the survey. We also present the missing observations (NAs) in this section.

The main results of the analysis are divided into two parts. The first part, [Annalysis -@sec-analysis], presents correlations between multiple variables and responses in the form of plots and graphs. The second part, [Detailed maps analysis -@sec-map], gives a visual representation of different responses with respect to Vaud's districts, which helps us categorize tourists geographically.
