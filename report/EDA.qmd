...
```{r Ratio of each canton of origin relatively to the number of tourist surveyed in Switzerland, fig.width=10, fig.height=6, echo=FALSE}
...
As we know that approximately 65% of Switzerland has as main language german, we thought that it would be interesting to know the distribution among swiss german visitors in the survey. Here is the plot showing the distribution per canton. 
...
Inside_Switzerland <- Inside_Switzerland %>%
  mutate(Categories = ifelse(Canton %in% swiss_german_cantons, "Swiss germans",
                             ifelse(Canton %in% c("Waadt"),"Vaud","Other")))
...
ggplot(counts,
  ...
  geom_text(aes(label=sprintf("%.2f%%", percentage)),
            position=position_stack(vjust=0.5)) + 
  labs(title = "Percentages of Swiss German, Vaud and other visitors",
       x = "Canton",
       y = "Percentage") +
  ylim(0, 90)
```

Over all the swiss citizen, we can see that 72% of them are coming from the swiss german part. 
...
For the ones with the package, most is spent on the package, then the eating expenses and finally with the overnight stay. This is explained by the fact that packages usually include transportations and activities, which have a pretty high cost in Switzerland.
...
Swiss travelers spend the most in total, followed by French and Amercian travelers.
