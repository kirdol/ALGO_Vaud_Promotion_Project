...
Note that the proportions displayed on the map represent percentages relative to the total number of visitors per district. However, due to the small numbers of visitors to certain origins and nationalities, these proportions should not be taken at face value for all areas. For better context, the total number of visitors per district is displayed alongside the percentage.
...
The following maps depict how people travel to the canton of Vaud. It shows the proportion of visitors of each regions that came with their children, alone or as a couple. Note that someone can be in multiple categories. For example, someone can be a couple and have children. He would then be counted in the couples and children categories.
...
# Alone proportion plot
summer_palette <- colorNumeric(palette = "Purples", domain = vaud$summer_proportion)
...
# Alone proportion plot
winter_palette <- colorNumeric(palette = "Purples", domain = vaud$winter_proportion)
...
Here is a map of the average and total spending by district. We can see that both the average and total spending is highest in the district of Riviera-Pays-d'Enhaut. We only take into account respondents with total expenses of under 30'000 CHF, in order to filter outliers.
...
spending_palette <- colorNumeric(palette = "Purples", domain = vaud$average_spending)
...
It is interesting to see that the tourists that came in the region of Montreux have spent the most on average, around 2600 CHF. The tourists that came in Lausanne spent closer to 2300 CHF while the rest of the regions, they spent between 1500 and 2000 CHF in average. This is a consequent difference in spending between the districts.

## Swiss tourists

Let's explore on a map the proportion of Swiss tourists in each districts. We seperated the Swiss-German and Swiss-French tourists to check for different preferences patterns. The proportion of travelers from Ticino and Grisons is negligible (< 1% proportion for any district) so we don't include them on the map.
...
Interestingly the Swiss-German tourists represents a high proportion of the tourists in the region of Jura Nord vaudois. The Swiss-French tourists in the other hand seems more present in the region of Aigle.

## French tourists

Let's explore on a map the most frequently visited districts by French tourists. We seperated the tourists from Île-de-France, Auvergne-Rhône-Alpes and the rest of France to check for different preferences patterns and proportions of french tourists in each regions.
...
We can see on the maps that the tourists from Île-de-France are well represented in the region of Lausanne while the tourists from Auvergne-Rhône-Alpes and the rest of France are also well represented in the region of Lausanne but also notably in the others regions.

## British tourists

You can find on the map below the most visited regions in proportions with to the total number of visitors in the region by British tourists. We seperated the tourists from London, the rest of England and Scotland to check for different preferences patterns. There were too few visitors from Northern Ireland and Wales to be included in the map.
...
There was a high proportion of visitors from London in the district Lavaux-Oron. We can't confirm that visitors from London are more interested in this district than other UK tourists, since the sample is so small.

## German tourists

You can find on the map below the most visited regions in proportions with to the total number of visitors in the region by German tourists. We seperated the tourists from Baden-Württemberg, Bayern and Nordrhein-Westfalen to check for different preferences patterns.
...
You can find on the map below the most visited regions in proportions with to the total number of visitors in the region by US tourists. We seperated the tourists from California and the rest of the US to check for different preferences patterns.
...
A high proportion of visitors of the Aigle district are from Belgium and Netherlands. Italy seems to prefer the region of Lausanne.
