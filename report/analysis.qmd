...
## Distribution of the days spent per month

In order to show the possible seasonality of visits, we decided to look at the number of days spent per month of the year in the canton of Vaud.
...
#plot
ggplot(nb_days_per_month, aes(x = month_name_column, y = total_hFerienDauer, fill = default_color)) +
  ...
  theme_minimal() +
  labs(x = "Months", y = "Sum Holiday Durations", title = "Total Holiday Duration by Month") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  guides(fill=FALSE)
```
 
We can clearly see two major periods in the year when there is an increase in the number of days spent in the canton of Vaud: March and July. This could be influenced by national holidays.
...
#plot
my_colors <- c("Holidays with only one overnight stay in Switzerland" = "#D73027",
"Round trip only within Switzerland (several overnight locations in Switzerland)" = "#F46D43",
"Round trip through Europe: only one overnight stay in Switzerland" = "#FDAE61",
"Round trip through Europe: several places to stay in Switzerland" = "#FEE08B")
...
## Nightstay category distribution per continent

Let's now look at the percentage of nightstay trips per category and continent. In the survey, visitors were asked to select the type of trip they took from four options, as explained in the legend of the graph. So here is the graph of the distribution of the type of trip for each continent.

```{r, echo = FALSE, message=FALSE, out.height="100%", out.width="100%", fig.width= 10, fig.height=8}

ggplot(trip_percentages, aes(x = F05_01_ENG, y = Percentages, fill = F16_ENG)) +
  geom_bar(stat = "identity") +
  labs(x = "Continents", y = "Percentages", fill = "Type of Trip") +
...
In general, we can see that most visitors from continents far away from Switzerland come to Switzerland for one or more nights during their trip in Europe. In addition, visitors living closer to Switzerland, such as Europeans, mostly stay for one night in Switzerland. This is probably due to the large number of visitors surveyed who comes from Switzerland but from other cantons or from neighboring countries such as Germany and France, all of which together account for around 50% of the visitors surveyed. We can also see that few people making a European tour would spend only one night in Switzerland.

## Global frequency of nightstays in switzerland

Here is the distribution of the frequencies of the number of nights spent during a trip in Switzerland.
...
ggplot(F20_freq, aes(x = F20, y = Count, fill = default_color)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
...
As the question asked to the visitors was the number of nights spent in Switzerland, it is difficult to know how many nights were spent in the canton of Vaud. We have therefore decided to use the total number of overnight stays in the country in our analysis. In addition, due to the structure of the survey, most of our data contained NAs (according to the answer to question F16, some people could not answer question F20, so their answers were reported as an NA). We have therefore decided to count the NAs as 1 overnight stay, as it is not possible to know if they only spent 1 day in Vaud without staying overnight.

We can therefore see that the majority of visitors generally spend only one night in the canton, with few people staying for more than a week. 
...
ggplot(distr_nightstays_per_district, aes(x = F20, y = F105_OrgCode, group = F105_OrgCode)) + 
  geom_density_ridges(fill = default_color, color = "white", alpha = 0.8) + 
...
## Distribution percentage nightstays in Switzerland over entire trip

Here is the distribution of the number of nights spent in Switzerland over the whole trip. 
...
ggplot(ratio_nightstays_CH_EU, aes(x = Percentage_Night_Stays_Switzerland)) +
  geom_histogram(binwidth = 10, fill = default_color, color = "black") +
  labs(
    title = "Distribution of Nightstays in Switzerland over Whole Trip",
    x = "Percentage nights spent in Switzerland",
    y = "Frequencies"
  ) +
  theme_minimal()
```

As this graph shows, we notice that most people will come to Switzerland for 20% of their trip, and few will spend more than 75% of their trip in the country.

## Distribution of arrivals per nationality

Now let's explore the distribution of the arrivals by nationality over the year. The full table of the arrivals can be found in the [Annex -@sec-data-arrivals].

```{r, fig.height=18, warning=FALSE, echo = FALSE}
### Deeper reasoning ###
...
# Visualize
ggplot(result, aes(x = month_name_column, y = `F05_02_ENG`, size = Count/total, color = default_color)) +
...
On this graph, we can see that the size of the dots represents the monthly number of arrivals of a nationality, scaled over the total number of arrivals of that particular country over the year, varies considerably according to the nationality. Unfortunately, due to the small number of data collected, some countries are represented only once in the whole year. In addition, arrivals vary according to country of origin: most countries have the same main arrival period in March and June, as seen before, but this is not the case for every country, showing differences in seasonality. 

## Average spending by travel group type

Here we have calculated the average total expenses in francs spent during a trip to the canton of Vaud per person, depending on who the respondent has traveled with. All the bars in this plot are non-mutually exclusive responses, meaning respondents could select multiple options, except for the bar "Only with partner", in which case the respondent only selected "traveled with partner".
...
American tourists have by far the highest percentage of tourists traveling alone as well as with a group of unknown people. This suggests they are more likely to be on a solo trip to Switzerland and also be on an organized group tour with other strangers.

:::

## Spending by district

It is interesting for us to combine the expense variables with the district ones in order to see in which category of expenses the travelers spent the most money for each district.
...
In all cases, the category for which the tourists spend the most amount of money is their overnight stays. It is followed mainly by food and drinks, transport, and souvenirs categories. Looking at the Ouest Lausannois district, we notice that travelers also spend on other expenses. 

Next, we will go through the spending for our seven main countries.

## Countries' spendings

We want to compare the total spending for our seven countries of interest compared to the spending per person.
...
ggplot(spendings_top7, aes(x = F05_02_ENG, y = Total_Spendings, fill = F05_02_ENG)) +
  ...
  scale_fill_manual(values = discrete_color, name = "Nationality")
```
...
We notice that by taking spending per person, Americans are the main spenders, followed by the British and the Spanish. When compared to the total spending, where Swiss, American, and French are the highest, the results for individual spending are more interesting to evaluate the nationality whose individuals spend the most in the canton of Vaud.

## Countries' spendings by district

Now, we will focus on the districts that these countries spend in. The aim is to see in which districts we find more nationalities and how much they spend in each district.
...
Top spenders: Swiss, French and British travelers.
...
Top spenders: Swiss, American and French travelers.
...
:::
  ...
  As a result, we notice that some countries, such as Spain, Germany, or the United Kingdom, are more present and spend more in some districts than others. French, Swiss, and Americans seem to be very present in most districts and have pretty high expenses in almost each one.